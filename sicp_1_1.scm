...
(print "\n\n  1.1.6 条件式と述語\n")
;; ※ここから翻訳を(https://github.com/minghai/sicp-pdf)こっちに変更した
;; テストを作成し, テストの結果により異なる命令を実行する方法が必要である.
;; (テスト = 条件分岐, 条件判定?)
;; Lispはそれらを表現するための特殊形式として, cond(condition)が用意されている.
;; また, この考え方をcase analysis(ケース分析, 事例分析)と呼ぶ.
(define (abs x)
  (cond ((> x 0) x)
        ((= x 0) 0)
        ((< x 0) (- x))
